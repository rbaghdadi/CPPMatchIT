...
Field<char, 200> filepath_out;
Field<unsigned char, MD5_DIGEST_LENGTH> md5_out;
...
extern "C" bool compare(const Element * const in1, const Element * const in2) {
    ...
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++) {
        if (computed_md5_1[i] != computed_md5_2[i]) {
            std::cerr << in1->get(&filepath_out) << " and " << in2->get(&filepath_out) << " do not match." <<
            std::endl;
            return false;
        }
    }
...
void setup(JIT *jit) {
    ...
    TransformStage xform = create_transform_stage(jit, file_to_md5, "file_to_md5", &filter_in,
                                                  &file_to_md5_out);
