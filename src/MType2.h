...
//    MStructType() {}
//
//    MStructType(mtype_code_t type_code, std::vector<MType *> field_types) : MType(type_code, 0) {
//        assert(is_struct_type());
//        unsigned int num_bits = 0;
//        // TODO this doesn't take into account any padding that can happen. Should call a sizeof operator on the correct field type and then sum
//        for (std::vector<MType*>::iterator iter = field_types.begin(); iter != field_types.end(); iter++) {
//            num_bits += (*iter)->get_bits();
//            underlying_types.push_back(*iter);
//        }
//        set_bits(num_bits);
//    }
//
