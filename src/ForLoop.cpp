...
llvm::AllocaInst *ForLoopCounters::get_loop_idx_alloc() {
    return loop_idx_alloc;
}

llvm::AllocaInst *ForLoopCounters::get_loop_bound_alloc() {
    return loop_bound_alloc;
}

llvm::AllocaInst *ForLoopCounters::get_return_idx_alloc() {
    return return_idx_alloc;
}

void ForLoopCounters::set_loop_bound_alloc(llvm::AllocaInst *max_loop_bound) {
    this->loop_bound_alloc = max_loop_bound;
}

void ForLoopCounters::codegen(JIT *jit, bool no_insert) {
...
llvm::Value *ForLoopCondition::get_loop_comparison() {
    return comparison;
}

void ForLoopCondition::set_loop_idx_alloc(llvm::AllocaInst *loop_idx) {
    this->loop_idx_alloc = loop_idx;
}

void ForLoopCondition::set_max_loop_bound_alloc(llvm::AllocaInst *max_loop_bound) {
...
void ForLoopCondition::codegen(JIT *jit, bool no_insert) {
...
void ForLoopIncrement::set_loop_idx_alloc(llvm::AllocaInst *loop_idx) {
    this->loop_idx_alloc = loop_idx;
}

void ForLoopIncrement::codegen(JIT *jit, bool no_insert) {
    assert(loop_idx_alloc);
    assert(!codegen_done);
    if (!no_insert) {
        jit->get_builder().SetInsertPoint(bb);
...
ForLoopIncrement *ForLoop::get_increment() {
    return loop_idx_increment;
}

ForLoopCondition *ForLoop::get_condition() {
    return condition;
}
