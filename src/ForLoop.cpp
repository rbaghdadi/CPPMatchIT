...
void ForLoop::codegen_counters(llvm::AllocaInst **counters, int num_counters_to_allocate) {
    jit->get_builder().SetInsertPoint(counter);
    for (int i = 0; i < num_counters_to_allocate; i++) {
        counters[i] = CodegenUtils::init_i64(jit, 0, "");
    }
}

void ForLoop::codegen_increment(llvm::AllocaInst *loop_idx) {
    increment->set_loop_idx_alloc(loop_idx);
    increment->codegen(jit);
}

void ForLoop::codegen_condition(llvm::AllocaInst *loop_bound, llvm::AllocaInst *loop_idx) {
    condition->set_max_loop_bound_alloc(loop_bound);
    condition->set_loop_idx_alloc(loop_idx);
...
llvm::BasicBlock *ForLoop::get_increment_bb() {
    return increment->get_basic_block();
}
