...
llvm::Value *SegmentationStage::compute_segments_per_setelement() {
    ...
    llvm::Value *total_array_size = as_i32(field_to_segment->get_fixed_size());
...
// TODO get rid of input parameter (and for the function below)
llvm::Value *SegmentationStage::compute_num_segments() {
    llvm::Value *segment_size_float =
            llvm::ConstantFP::get(llvm_float, (float)segment_size);
    llvm::Value *overlap_float =
            llvm::ConstantFP::get(llvm_float, overlap);
    llvm::Value *total_array_size = jit->get_builder().CreateUIToFP(codegen_llvm_mul(jit, as_i32(field_to_segment->get_fixed_size()),
                                                     codegen_llvm_load(jit, loop->get_loop_bound(), 4)), llvm_float);
    llvm::Value *numerator =
            jit->get_builder().CreateFSub(total_array_size, jit->get_builder().CreateFMul(segment_size_float,
                                                                                               overlap_float));
    llvm::Value *denominator =
...
// combine with above
llvm::Value *SegmentationStage::compute_num_segments(BaseField *output_field, llvm::Value *loop_bound) {
    llvm::Value *segment_size_float =
            llvm::ConstantFP::get(llvm_float, (float)segment_size);
    llvm::Value *overlap_float =
            llvm::ConstantFP::get(llvm_float, overlap);
